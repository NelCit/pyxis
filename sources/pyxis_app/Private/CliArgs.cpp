    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (Equals(arg, "--headless")) {
            ...
            if (i + 1 >= argc || !ParseInt32(argv[i + 1], out.adapterIndex)) {
                out.invalid = true; out.invalidArg = arg; return out;
            }
            ++i;
        ...
        // ---- §26 config + scene -----------------------------------------
            ...
            if (!TakeValue(argc, argv, i, out.configPath))   { out.invalid = true; out.invalidArg = arg; return out; }
        } else if (Equals(arg, "--scene")) {
            if (!TakeValue(argc, argv, i, out.scenePath))    { out.invalid = true; out.invalidArg = arg; return out; }
        } else if (Equals(arg, "--camera")) {
            if (!TakeValue(argc, argv, i, out.cameraSdfPath)){ out.invalid = true; out.invalidArg = arg; return out; }

        // ---- §26 render / output overrides ------------------------------
        } else if (Equals(arg, "--width")) {
            if (i + 1 >= argc || !ParseUInt32(argv[i + 1], out.width))   { out.invalid = true; out.invalidArg = arg; return out; }
            ++i;
        } else if (Equals(arg, "--height")) {
            if (i + 1 >= argc || !ParseUInt32(argv[i + 1], out.height))  { out.invalid = true; out.invalidArg = arg; return out; }
            ++i;
        } else if (Equals(arg, "--samples")) {
            if (i + 1 >= argc || !ParseUInt32(argv[i + 1], out.samples)) { out.invalid = true; out.invalidArg = arg; return out; }
            ++i;
        } else if (Equals(arg, "--seed")) {
            if (i + 1 >= argc || !ParseUInt32(argv[i + 1], out.seed))    { out.invalid = true; out.invalidArg = arg; return out; }
            ++i;
        } else if (Equals(arg, "--output")) {
            if (!TakeValue(argc, argv, i, out.outputPath))   { out.invalid = true; out.invalidArg = arg; return out; }
        } else if (Equals(arg, "--profile")) {
            if (!TakeValue(argc, argv, i, out.profilePath))  { out.invalid = true; out.invalidArg = arg; return out; }

        // ---- M1 viewer extras -------------------------------------------
        } else if (Equals(arg, "--screenshot")) {
            if (!TakeValue(argc, argv, i, out.screenshotPath)) { out.invalid = true; out.invalidArg = arg; return out; }
