...
struct CliArgs {
  ...
  // V2.A.14 — CPU-side BCn texture compression. The GpuScene texture
  // decoder runs stb_dxt over each stbi_load-decoded RGBA8 buffer
  // before GPU upload (BC1 for baseColor/emission, BC5 for normal
  // maps, BC4 for roughness/metallic) — saves ~6× VRAM on the lobby
  // texture set. **On by default** since the V2.A.14 follow-up;
  // `--no-compress-textures` opts out for diagnostic builds that need
