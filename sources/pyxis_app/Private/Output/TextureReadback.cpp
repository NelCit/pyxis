...
std::expected<TextureReadback, std::string>
TextureReadback::RecordCopy(nvrhi::IDevice*      device,
    ...
    if (!staging) {
        return std::unexpected{
            std::string{"TextureReadback::RecordCopy: createStagingTexture failed"}};
    }

    commandList->copyTexture(staging.Get(), nvrhi::TextureSlice{},
                             source,         nvrhi::TextureSlice{});
...
std::expected<void, std::string> TextureReadback::Map() noexcept {
    ...
    if (_mapped != nullptr) {
        return std::unexpected{
            std::string{"TextureReadback::Map: already mapped"}};
    }
    _mapped = _device->mapStagingTexture(_staging.Get(),
                                         nvrhi::TextureSlice{},
                                         nvrhi::CpuAccessMode::Read,
                                         &_rowPitch);
    if (_mapped == nullptr) {
        return std::unexpected{
            std::string{"TextureReadback::Map: mapStagingTexture failed"}};
    }
    return {};
}
