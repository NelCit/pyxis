...
// canonical answer is the latter — promote to RGBA16F + OCIO once
// the path tracer produces HDR values. Until then M3's path-trace
// closesthit writes barycentrics-as-colour (linear, in [0,1]) which
// converts cleanly here.
void Bgra8ToRgbaFloat(uint32_t width, uint32_t height, const void* bgra8Pixels,
                      std::size_t rowPitchBytes, std::vector<float>& outRgba) {
...
std::expected<void, std::string> WriteExrBgra8(std::string_view filePath, uint32_t width,
                                               uint32_t height, const void* bgra8Pixels,
                                               std::size_t rowPitchBytes) noexcept {
  if (filePath.empty())
    return std::unexpected{std::string{"WriteExrBgra8: empty path"}};
  if (bgra8Pixels == nullptr)
    return std::unexpected{std::string{"WriteExrBgra8: null source"}};
  if (width == 0 || height == 0)
    return std::unexpected{std::string{"WriteExrBgra8: zero dim"}};
  ...
  const int saveResult = SaveEXR(rgba.data(), static_cast<int>(width), static_cast<int>(height),
                                 /*components*/ 4,
