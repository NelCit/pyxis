...
DeviceManagerCreateStatus VkDeviceManagerHeadless::Bringup(
    const DeviceCreationParams& params, const Resolution& initialBackbuffer) noexcept {
  ...
  {
    log.Warn(log::PLATFORM, std::string{"VkDeviceManagerHeadless: framesInFlight="}
                                + std::to_string(params.framesInFlight) + " out of range ["
                                + std::to_string(MIN_FIF) + ", " + std::to_string(MAX_FIF)
  ...
  // render at" — useful when scrubbing logs but not load-bearing.
  {
    ...
    msg += std::to_string(initialBackbuffer.width) + "x" + std::to_string(initialBackbuffer.height);
...
nvrhi::IDevice* VkDeviceManagerHeadless::GetDevice() const noexcept {
  return _nvrhiDevice.Get();
}
const AdapterInfo& VkDeviceManagerHeadless::GetAdapterInfo() const noexcept {
  return _adapter;
}
uint32_t VkDeviceManagerHeadless::GetFramesInFlight() const noexcept {
