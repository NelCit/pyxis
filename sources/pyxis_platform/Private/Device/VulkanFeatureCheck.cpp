...
bool HasExtension(const std::vector<VkExtensionProperties>& props, const char* name) noexcept {
  return std::any_of(props.begin(), props.end(), [&](const VkExtensionProperties& prop) {
...
bool QueryAdapterFeatures(VkPhysicalDevice device, AdapterInfo& outInfo) noexcept {
  ...
  {
    case 0x10DE:
      outInfo.vendor = AdapterVendor::Nvidia;
      break;
    case 0x1002:
      outInfo.vendor = AdapterVendor::Amd;
      break;
    case 0x8086:
      outInfo.vendor = AdapterVendor::Intel;
      break;
    case 0x5143:
      outInfo.vendor = AdapterVendor::Qualcomm;
      break;
    default:
      outInfo.vendor = AdapterVendor::Unknown;
      break;
  }
  ...
  switch (props.deviceType)
  {
    case VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU:
      outInfo.type = AdapterType::Discrete;
      break;
    case VK_PHYSICAL_DEVICE_TYPE_INTEGRATED_GPU:
      outInfo.type = AdapterType::Integrated;
      break;
    case VK_PHYSICAL_DEVICE_TYPE_VIRTUAL_GPU:
      outInfo.type = AdapterType::Virtual;
      break;
    case VK_PHYSICAL_DEVICE_TYPE_CPU:
      outInfo.type = AdapterType::Cpu;
      break;
    default:
      outInfo.type = AdapterType::Other;
      break;
  }
  ...
  for (uint32_t i = 0; i < memProps.memoryHeapCount; ++i)
  {
    ...
    {
      outInfo.totalDeviceLocalBytes = std::max(outInfo.totalDeviceLocalBytes, heap.size);
    }
  }
  ...
  outInfo.supportsTimelineSemaphore = HasExtension(exts, VK_KHR_TIMELINE_SEMAPHORE_EXTENSION_NAME);
  outInfo.supportsHostQueryReset = HasExtension(exts, VK_EXT_HOST_QUERY_RESET_EXTENSION_NAME);
  outInfo.supportsMaintenance4 = HasExtension(exts, VK_KHR_MAINTENANCE_4_EXTENSION_NAME);
  outInfo.supportsMemoryBudget = HasExtension(exts, VK_EXT_MEMORY_BUDGET_EXTENSION_NAME);
  ...
  if (!outInfo.supportsRayTracingPipeline)
    missing(VK_KHR_RAY_TRACING_PIPELINE_EXTENSION_NAME);
  if (!outInfo.supportsAccelerationStructure)
    missing(VK_KHR_ACCELERATION_STRUCTURE_EXTENSION_NAME);
  if (!outInfo.supportsBufferDeviceAddress)
    missing(VK_KHR_BUFFER_DEVICE_ADDRESS_EXTENSION_NAME);
  if (!outInfo.supportsDescriptorIndexing)
    missing(VK_EXT_DESCRIPTOR_INDEXING_EXTENSION_NAME);
  if (!outInfo.supportsTimelineSemaphore)
    missing(VK_KHR_TIMELINE_SEMAPHORE_EXTENSION_NAME);
  if (!outInfo.supportsHostQueryReset)
    missing(VK_EXT_HOST_QUERY_RESET_EXTENSION_NAME);
  if (!outInfo.supportsMaintenance4 && props.apiVersion < VK_API_VERSION_1_3)
    missing(VK_KHR_MAINTENANCE_4_EXTENSION_NAME);
  if (!outInfo.supportsShaderInt64)
    missing("shaderInt64");
  if (!outInfo.supportsSync2)
    missing("Vulkan 1.3 core (synchronization2)");
