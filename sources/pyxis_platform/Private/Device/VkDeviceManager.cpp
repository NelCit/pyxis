...
const char* StatusName(DeviceManagerCreateStatus status) noexcept {
  switch (status)
  {
    case DeviceManagerCreateStatus::Ok:
      return "Ok";
    case DeviceManagerCreateStatus::NoVulkanDriver:
      return "NoVulkanDriver";
    case DeviceManagerCreateStatus::NoCompatibleAdapter:
      return "NoCompatibleAdapter";
    case DeviceManagerCreateStatus::FeatureMissing:
      return "FeatureMissing";
    case DeviceManagerCreateStatus::InstanceCreationFailed:
      return "InstanceCreationFailed";
    case DeviceManagerCreateStatus::DeviceCreationFailed:
      return "DeviceCreationFailed";
    case DeviceManagerCreateStatus::SurfaceCreationFailed:
      return "SurfaceCreationFailed";
    case DeviceManagerCreateStatus::SwapchainCreationFailed:
      return "SwapchainCreationFailed";
    case DeviceManagerCreateStatus::OutOfMemory:
      return "OutOfMemory";
    case DeviceManagerCreateStatus::Unknown:
      return "Unknown";
  }
  return "?";
}
...
VkInstance CreateInstance(bool enableValidation, std::string_view appName, uint32_t appVersion,
  ...
  {
    if (outStatus)
      *outStatus = DeviceManagerCreateStatus::InstanceCreationFailed;
...
VkDeviceManager::VkDeviceManager(const DeviceCreationParams& params, IWindow* window,
...
DeviceManagerCreateStatus VkDeviceManager::Bringup(const DeviceCreationParams& params,
  ...
  for (uint32_t i = 0; i < deviceCount; ++i)
  {
    ...
    if (rank > bestRank || (rank == bestRank && info.totalDeviceLocalBytes > bestVram))
    {
      bestRank = rank;
  ...
  // capability for graphics pipelines that use SV_VertexID /
  // SV_InstanceID-driven vertex pulls. M3's RT pipeline doesn't use
  // it, but enabling the v1.1 feature here keeps the device manager
  // forward-compatible with M5+ rasterization passes (debug
  // overlays, AOV resolves) without re-creating the device.
  // Without this VUID-VkShaderModuleCreateInfo-pCode-08740 fires at
...
bool VkDeviceManager::CreateSwapchain(uint32_t width, uint32_t height) noexcept {
  ...
  vkGetPhysicalDeviceSurfacePresentModesKHR(_physicalDevice, _surface, &presentModeCount,
                                            presentModes.data());
  ...
  for (uint32_t i = 0; i < actualCount; ++i)
  {
    ...
    nvrhi::TextureHandle textureHandle = _nvrhiVulkan->createHandleForNativeTexture(
        nvrhi::ObjectTypes::VK_Image, nvrhi::Object(_swapchainImages[i]), tdesc);
    if (!textureHandle)
    {
...
nvrhi::ITexture* VkDeviceManager::GetBackbuffer(uint32_t index) const noexcept {
  if (index >= _swapchainTextures.size())
    return nullptr;
...
void VkDeviceManager::BeginFrame() {
  ...
  const VkResult acquireResult = vkAcquireNextImageKHR(_device, _swapchain, UINT64_MAX, _acquireSem,
                                                       VK_NULL_HANDLE, &_currentImage);
...
IDeviceManager* CreateWindowedDeviceManager(const DeviceCreationParams& params, IWindow* window,
                                            const Resolution& initialBackbuffer,
