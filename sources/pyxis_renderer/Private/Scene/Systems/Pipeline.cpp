...
void RegisterSystems(flecs::world& world) {
  world.system("System_UploadDirtyTextures").kind<PhaseUploadTextures>().run([](flecs::iter& iter) {
    RunUploadDirtyTextures(iter);
  });
  ...
  world.system("System_ExtractDirtyMeshes").kind<PhaseExtractMeshes>().run([](flecs::iter& iter) {
    RunExtractDirtyMeshes(iter);
  });

  world.system("System_BuildDirtyBlas").kind<PhaseBuildBlas>().run([](flecs::iter& iter) {
    RunBuildDirtyBlas(iter);
  });

  world.system("System_RebuildTlas").kind<PhaseRebuildTlas>().run([](flecs::iter& iter) {
    RunRebuildTlas(iter);
  });

  world.system("System_UpdateBindlessTable").kind<PhaseUpdateBindless>().run([](flecs::iter& iter) {
    RunUpdateBindlessTable(iter);
  });

  world.system("System_ClearDirtyFlags").kind<PhaseClearDirty>().run([](flecs::iter& iter) {
    RunClearDirtyFlags(iter);
  });
}
