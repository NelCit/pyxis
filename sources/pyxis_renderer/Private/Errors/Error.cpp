...
Error MakeError(ErrorKind kind, const char* file, int line, const char* fmt, ...) noexcept {
  Error err;
  err.kind = kind;
  ...
  // Caller-supplied human-readable message.
  {
    va_list args;
    va_start(args, fmt);
    const int written = std::vsnprintf(err.message.data.data(), ErrorMessage::CAPACITY,
                                       fmt != nullptr ? fmt : "", args);
    va_end(args);
  ...
  {
    const int written = std::snprintf(err.source.data.data(), ErrorMessage::CAPACITY, "%s:%d",
