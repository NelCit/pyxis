// Pyxis renderer — AmbientOcclusionPass (RTX-alignment design, WP2-signals).
...
nvrhi::ShaderHandle LoadSpirv(nvrhi::IDevice* device, std::string_view path,
  ...
  return LoadSpirvShader(device, path, stage, entry, "AmbientOcclusionPass");
}
...
// One RT pipeline + SBT per camera projection mode — same scheme as
// RaytracedGBufferPass. This pass never touches OpenPBR closures (RTAO
// needs only geometry), so PROJECTION_MODE is the only specialized
// constant; MAX_RAY_RECURSION / REFL_MAX_SEGMENTS / OPENPBR_FEATURE_MASK
// aren't even declared in this shader's module (it doesn't include
// shading.slang).
PipelineVariant BuildPipelineVariant(nvrhi::IDevice* device, nvrhi::IBindingLayout* sceneLayout,
                                     ...
                                     nvrhi::IShader* miss, nvrhi::IShader* anyHit,
  ...
  if (!specRaygen)
  {
    log.Error(log::RENDER, "AmbientOcclusionPass: createShaderSpecialization failed (projectionMode="
  ...
  // Any-hit-only hit group: the occlusion ray always carries
  // RAY_FLAG_SKIP_CLOSEST_HIT_SHADER, so closestHitShader stays null
  // (never invoked) — legal per nvrhi::rt::PipelineHitGroupDesc.
  ...
  if (!variant.pipeline)
  {
    log.Error(log::RENDER, "AmbientOcclusionPass: createRayTracingPipeline failed (projectionMode="
                              + std::to_string(projectionMode) + ")");
    return {};
  }
  ...
  {
    log.Error(log::RENDER, "AmbientOcclusionPass: createShaderTable failed (projectionMode="
                              + std::to_string(projectionMode) + ")");
    return {};
  }
  variant.shaderTable->setRayGenerationShader("RayGenMain");
  variant.shaderTable->addMissShader("MissMain");  // miss-index 0: the occlusion ray's only miss.
...
AmbientOcclusionPass::AmbientOcclusionPass(nvrhi::IDevice* device, GpuScene& scene,
                                           SceneBindings& sceneBindings)
    : _device(device), _scene(&scene), _sceneBindings(&sceneBindings) {
  if (!_sceneBindings->IsOperational())
  {
    Logging::Get().Error(log::RENDER,
                         "AmbientOcclusionPass: SceneBindings not operational; pass will skip");
    return;
  }

  const AssetLocator locator;
  const Path raygenPath = locator.LocateResource("shaders/ambient_occlusion_raygen.spv");
  const Path missPath = locator.LocateResource("shaders/ambient_occlusion_miss.spv");
  const Path anyHitPath = locator.LocateResource("shaders/ambient_occlusion_anyhit.spv");
  ...
  if (!_raygenShader || !_missShader || !_anyHitShader)
  {
    Logging::Get().Error(log::RENDER, "AmbientOcclusionPass: shader load failed; pass will skip");
    return;
  }
  ...
  layoutDesc.bindings = {
      ...
      nvrhi::BindingLayoutItem::Texture_UAV(2),           // 2 gAo
  };
  _passLayout = _device->createBindingLayout(layoutDesc);
  if (!_passLayout)
  {
    Logging::Get().Error(log::RENDER, "AmbientOcclusionPass: createBindingLayout(Set 1) failed");
    return;
  }

  PipelineVariant perspective = BuildPipelineVariant(
      _device, _sceneBindings->Layout(), _passLayout, _raygenShader, _missShader, _anyHitShader,
      /*projectionMode=*/0u);
  ...
  _shadersOk = true;
  Logging::Get().Info(log::RENDER, "AmbientOcclusionPass: initialised (RT pipeline + SBT ready)");
}

AmbientOcclusionPass::~AmbientOcclusionPass() = default;

nvrhi::ITexture* AmbientOcclusionPass::EnsureOutput(uint32_t width, uint32_t height) {
  ...
  desc.debugName = "AmbientOcclusion.gAo";
  ...
  if (!_output)
  {
    Logging::Get().Error(log::RENDER, "AmbientOcclusionPass: createTexture(gAo, "
...
void AmbientOcclusionPass::EnsureProjectionPipeline() {
  ...
  PipelineVariant built =
      BuildPipelineVariant(_device, _sceneBindings->Layout(), _passLayout, _raygenShader,
                           _missShader, _anyHitShader, static_cast<uint32_t>(variant));
...
nvrhi::BindingSetHandle AmbientOcclusionPass::GetOrCreateBindingSet(
    nvrhi::IBuffer* visibility, nvrhi::ITexture* normalRoughness) {
...
void AmbientOcclusionPass::Execute(nvrhi::ICommandList* commandList, const PassContext& context) {
  if (commandList == nullptr || context.targets == nullptr || context.settings == nullptr)
    return;
  // WP2-final — RealTimeQuality.passMask bit 2 gates this signal pass;
  // default 0x1F runs every pass (image-identical to before this gate).
  if ((context.settings->realTimeQuality.passMask & 0x04u) == 0u)
    return;
