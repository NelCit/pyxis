...
TEST(Error, ErrorMessageIsTriviallyCopyable) {
    ...
    static_assert(ErrorMessage::CAPACITY == 240,
                  "Bumping CAPACITY is a major-version break (§22).");
}
...
TEST(PyxisError, PopulatesKindMessageAndSource) {
    const Error err = PYXIS_ERROR(ErrorKind::FileNotFound,
                                  "no such file: %s",
                                  "/tmp/missing.usd");
...
TEST(PyxisError, MessageTruncatesWithTrailingEllipsis) {
    ...
    EXPECT_EQ(view.size(), ErrorMessage::CAPACITY - 1);  // CAPACITY-1 visible bytes (vsnprintf null terminator at end)
...
TEST(Expected, HoldsErrorOnUnexpected) {
    const Expected<int> failed = std::unexpected{
        PYXIS_ERROR(ErrorKind::OutOfMemoryGpu, "GPU OOM at %u MiB", 4096u)
    };
...
TEST(Expected, VoidSpecialisationCompiles) {
    ...
    const Expected<void> failed = std::unexpected{
        PYXIS_ERROR(ErrorKind::DeviceLost, "lost")
    };
    ASSERT_FALSE(failed.has_value());
...
Expected<void> AlwaysFails() {
    return std::unexpected{
        PYXIS_ERROR(ErrorKind::NotImplemented, "stub: %s", "AlwaysFails")
    };
}
